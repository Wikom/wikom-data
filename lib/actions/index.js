 ...
 * @return {Promise}
 */
var upload = exports.upload = function upload(_ref19) {
    var url = _ref19.url,
        _ref19$data = _ref19.data,
        data = _ref19$data === undefined ? {} : _ref19$data,
        _ref19$primaryKey = _ref19.primaryKey,
        primaryKey = _ref19$primaryKey === undefined ? 'id' : _ref19$primaryKey;
    return function () {
        var req = data.hasOwnProperty(primaryKey) ? baseRequest({ method: 'PUT', url: url + '/' + (0, _findInObject2.default)(primaryKey, data), data: data }) : baseRequest({ method: 'POST', url: url });
 ...
 * @return {Promise}
 */
var directDelete = exports.directDelete = function directDelete(_ref21) {
    var url = _ref21.url,
        _ref21$refresh = _ref21.refresh,
        refresh = _ref21$refresh === undefined ? null : _ref21$refresh;
    return function (dispatch) {
        return dispatch(executeDelete({ url: url })).then(function (result) {
            return refresh !== null ? dispatch(refreshData({ name: refresh })) : Promise.resolve('success');
 ...
 * @return {Promise}
 */
var confirmAndDelete = exports.confirmAndDelete = function confirmAndDelete(_ref22) {
    var url = _ref22.url,
        _ref22$refresh = _ref22.refresh,
        refresh = _ref22$refresh === undefined ? null : _ref22$refresh,
        _ref22$message = _ref22.message,
        message = _ref22$message === undefined ? 'Sind Sie sicher, dass Sie diesen Eintrag löschen möchten?' : _ref22$message;
    return function (dispatch) {
        return confirm(message) ? dispatch(directDelete({ url: url, refresh: refresh })) : Promise.resolve('canceled');
    };
};