...
var DataProvider = function (_Component) {
    ...
    _createClass(DataProvider, [{
        ...
        value: function render() {
            return this.props.onlyLoaded && this.props.isLoading ? _react2.default.createElement(_reactLoading2.default, null) : _react2.default.createElement(
                'span',
                null,
                this.props.children
            );
...
var mapChildren = function mapChildren(Child, state, dataProp, isLoadingProp, name, rest) {
    var _extends2;

    var storeData = state.data && state.data[name] || null;
    var data = Child.props.hasOwnProperty(dataProp) ? Child.props[dataProp] : false;
    var overwrite = Child.props.hasOwnProperty('overwriteInitial') && Child.props.overwriteInitial;
    if (data) {
            ...
            case "object":
                if (!Array.isArray(data)) {
                    if (!Array.isArray(storeData)) {
                        if (overwrite) {
                            data = _extends({}, data, storeData);
                        } else {
                            data = _extends({}, storeData, data);
                        }
                    } else {
                        if (overwrite) {
                            data = storeData;
    ...
    return _react2.default.cloneElement(Child, _extends((_extends2 = {}, _defineProperty(_extends2, isLoadingProp, !state.queries[name] || state.queries[name].isPending === true), _defineProperty(_extends2, dataProp, data), _defineProperty(_extends2, 'pagination', state.data && state.data.pagination && state.data.pagination[name] || null), _extends2), rest));
};
...
var mapStateToProps = function mapStateToProps(state, _ref) {
    var children = _ref.children,
        ...
        rest = _objectWithoutProperties(_ref, ['children', 'name', 'url', 'force', 'dataProp', 'isLoadingProp', 'refresh']);
    ...
    return {
        ...
        children: _react2.default.Children.map(children, function (Child) {
            return mapChildren(Child, state, dataProp, isLoadingProp, name, rest);
        })
    };
};
